...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not current_place.is_hive :
            if current_place.bees and self.min_range <= transition <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        #Making a copy of the list and iterating over that to avoid skipping elements
        new_copied_list = list(self.place.bees)
        Ant.reduce_health(self, amount)
        for bee in new_copied_list:
            if self.health <= 0:
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if not gamestate.queen_is_placed:
            gamestate.queen_is_placed = True
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        place = self.place 
        even_state = gamestate.time % 2

        if self.blocked():
            self.sting(place.ant)
        elif self.health > 0 and destination is not None:
            if self.back > 0:
                if self.slowed == 0:
                    self.back -= 1 
                    for i in range(self.scared):
                        self.move_to(place.entrance)
                elif self.slowed > 0:
                    if even_state == 0:
                        self.slowed -= 1
                        self.back -= 1
