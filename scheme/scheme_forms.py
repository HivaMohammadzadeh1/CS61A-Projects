...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        expression = do_lambda_form(Pair(signature.rest, expressions.rest),env)
        env.define(signature.first, expression)
...
def make_let_frame(bindings, env):
    ...
    while bindings: 
        ...
        validate_form(one_binding,2,2)
        names = Pair(one_binding.first,names)
        values = Pair(scheme_eval(one_binding.rest.first,env),values)  
