...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while max (score0, score1) < goal:
        if who == 0:
            numberOfRoles = strategy0(score0, score1)
            score0 += take_turn(numberOfRoles, score1, dice) 
            score0 += hog_pile(score0, score1)

        else:
            numberOfRoles = strategy1(score1, score0)
            score1 += take_turn(numberOfRoles, score0, dice)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if who == 0:
            current0 = score0 
            difference0 = current0 - last
            if (difference0 > high):
                print( difference0 , "point(s)! That's a record gain for Player 0!")
                return announce_highest(who, current0, difference0)
            return announce_highest(who, current0, running_high)
        else:
            current1 = score1
            difference1 = current1 - last
            if (difference1 > high):
                print( difference1 , "point(s)! That's a record gain for Player 1!")
                return announce_highest(who, current1, difference1)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    n , best_value_so_far = 1 , 0
    averaged_roll_dice = make_averaged(roll_dice, trials_count)

    while n <= 10:
        max_n = averaged_roll_dice(n, dice)
        if max_n > best_value_so_far:
            best_value_so_far , max_rolls = max_n, n
        n += 1
    return max_rolls
